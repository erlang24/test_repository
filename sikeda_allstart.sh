...
gnome-terminal -- bash -c "docker exec -it xiaoche-humble /bin/bash -c 'source skd_pkg/install/setup.bash; ros2 launch chcnav_driver chanav_node.launch.py; exec bash'"

# 进入第二个 Docker 容器并启动 GPS 服务
echo "进入容器,并启动 GPS 服务......"
gnome-terminal -- bash -c "docker exec -it xiaoche-humble /bin/bash -c 'source pm_v2x/install/setup.bash; ros2 launch gps_to_xyz sikeda.launch.py; exec bash'"
